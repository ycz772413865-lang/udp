...
def log_event(role, event):
    log_queue.put((datetime.datetime.now().isoformat(), role, event))

def log_writer():
    global log_running
    while log_running:
        try:
            ts, role, event = log_queue.get(timeout=0.5)
            with open(LOG_FILE, 'a', encoding='utf-8') as f:
                portalocker.lock(f, portalocker.LOCK_EX)
                f.write(f"{ts} [{role}] {event}\n")
                f.flush()
...
def main():
    ...
    while not log_queue.empty():
        try:
            ts, role, event = log_queue.get_nowait()
            with open(LOG_FILE, 'a', encoding='utf-8') as f:
                portalocker.lock(f, portalocker.LOCK_EX)
                f.write(f"{ts} [{role}] {event}\n")
                f.flush()
